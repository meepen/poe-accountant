locals {
  ...
  full_cdn_domain_name = "${var.cdn_subdomain}.${var.cloudflare_zone_name}"
  ...
  workers = {
    "background-processor" = {
      ...
      env = {
        ...
        "CDN_BASE_URL"                = "https://${local.full_cdn_domain_name}"
...
module "frontend" {
  ...
  environment_variables = {
    ...
    VITE_CDN_BASE_URL = "https://${local.full_cdn_domain_name}"
  }
}
...
module "r2_cdn" {
  ...
  public_custom_domain = local.full_cdn_domain_name
  zone_id              = var.cloudflare_zone_id
}
...
locals {
  cloudflare_permissions_map = {
    for permission in data.cloudflare_api_token_permission_groups_list.all.result : permission.name => permission.id...
  }
}
...
resource "cloudflare_api_token" "wrangler_deploy" {
  ...
  policies = [{
    ...
    permission_groups = [
      { id = local.cloudflare_permissions_map["Workers Scripts Write"][0] },
      { id = local.cloudflare_permissions_map["Workers R2 Storage Write"][0] },
      { id = local.cloudflare_permissions_map["Account Settings Read"][0] },
      { id = local.cloudflare_permissions_map["User Details Read"][0] },
      { id = local.cloudflare_permissions_map["Memberships Read"][0] },
      { id = local.cloudflare_permissions_map["Workers Routes Write"][0] },
      { id = local.cloudflare_permissions_map["Zone Read"][0] },
      { id = local.cloudflare_permissions_map["DNS Write"][0] }
    ]
